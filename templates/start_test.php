...
$instruction = ( isset($info['instruction']) && $info['instruction'] ) ? $info['instruction']:'';
	...
	<div class="card questions-form">
		...
		<div class="intro">
			...
			<form id="questionsForm" method="POST" action="<?=$siteRootURL?>inc/submit.php">
				...
				<input type="hidden" name="result" value="<?=$siteRootURL?>result.php">
							...
							<li class="question-item" data-index="<?= $ctr ?>" data-answered="">
								...
								<div class="choices">
									<?php for($score=0; $score<5; $score++) { 
										...
										<label for="<?= $field_id ?>" class="radio-style scoreInpt<?=$score?>">
											<?php if ($score==0) { ?>
											<div class="ins">Strongly Disagree</div>	
											<?php } ?>
											<?php if ($score==4) { ?>
											<div class="ins">Strongly Agree</div>	
											<?php } ?>
